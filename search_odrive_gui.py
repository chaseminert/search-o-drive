...
def update_full_selection(index=0, initialize=False):
    ...
    full_str = display_to_company[
        display_str]  # "Really Long Company Name..." will map to "Really Long Company Name LLC" for example
...
def get_path_from_company(company_name,
                          full=False):  # full means get the path to the company, otherwise it just gets the base
    # directory
...
def selection_screen_display():
    ...
    docuware_inactive_button = ctk.CTkButton(buttons_frame, text="Docuware Inactive",
                                             command=lambda: set_docuware_query(2),
                                             font=button_font, border_width=2, border_color=TCS_RED, fg_color=TCS_BLUE,
                                             text_color="white", hover_color="#000040", corner_radius=10)
...
def main_program_display():
    ...
    credit_label = ctk.CTkLabel(very_top_frame, text="Created by Chase Minert", font=("Arial", 12), text_color="grey40",
                                anchor='w')
    ...
    search_hint_element = ctk.CTkLabel(search_frame, text="Enter Company Name", font=("Arial", 12, 'bold'),
                                       text_color="white")
    ...
    if docuware_query:
        checkbox_text = ctk.CTkLabel(checkbox_frame, text="Search by DOT#", font=("Arial", 12, "bold"),
                                     text_color="white")
        ...
        checkbox_text = ctk.CTkLabel(checkbox_frame, text="Search by DQF", font=("Arial", 12, "bold"),
                                     text_color="white")
        checkbox_text.grid(row=0, column=0, padx=10, ipady=0)
    ...
    full_selection_name = ctk.CTkLabel(selection_frame, text="", font=("Arial", 14), wraplength=max_len, anchor="nw",
                                       justify="left", width=max_len)
    full_selection_name.grid(row=0, column=1, sticky='NW', pady=(2, 5))

    full_selection_dot = ctk.CTkLabel(selection_frame, text="", font=("Arial", 14), wraplength=max_len, anchor="nw",
                                      justify='left')
    full_selection_dot.grid(row=1, column=1, sticky='NW', pady=(0, 5))

    full_selection_type = ctk.CTkLabel(selection_frame, text="", font=("Arial", 14), wraplength=max_len, anchor="nw",
                                       justify='left')
    ...
    ctk.CTkFrame(main_window, width=200, height=2000, fg_color="black")
    ...
    main_window.bind("<Return>", lambda event, window=main_window: select_wrapper(window))
    # Bind events
    search_entry.bind("<KeyRelease>", on_search_change)
    suggestions_listbox.bind("<ButtonRelease-1>", lambda event, window=main_window: select_wrapper(window))
